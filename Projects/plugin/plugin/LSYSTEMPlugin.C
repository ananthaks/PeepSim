...
static PRM_Name angle("angle", "Angle");
static PRM_Name stepSize("step", "StepSize");
static PRM_Name numIteration("num", "Iterations");
static PRM_Name grammar("grammar", "Grammar");
...
static PRM_Default numIterationDefault(2);
static PRM_Default grammarDefault(0.0, "Sup");
...
// Similarly add all the other parameters in the template format here
	...
	PRM_Template(PRM_FLT, PRM_Template::PRM_EXPORT_MIN, 1, &angle, &angleDefault, 0),
	PRM_Template(PRM_FLT, PRM_Template::PRM_EXPORT_MIN, 1, &stepSize, &stepSizeDefault, 0),
	PRM_Template(PRM_INT, PRM_Template::PRM_EXPORT_MIN, 1, &numIteration, &numIterationDefault, 0),
...
SOP_Lsystem::cookMySop(OP_Context &context)
{
	...
	int numIterations;
	numIterations = NUMITERATION(now);

	UT_String grammerFile;
	GRAMMAR(grammerFile, now);
	...
	//myplant.loadProgramFromString(grammerFile.toStdString());

	myplant.loadProgram(grammerFile.toStdString());
	...
	if (boss->opStart("Building LSYSTEM"))
	{
		...
		for (int i = 0; i < branches.size(); ++i) {
			...
			UT_Vector3F startPos = UT_Vector3F(branches[i].first[0], branches[i].first[2], branches[i].first[1]);
			gdp->setPos3(ptoff, startPos);
			poly->appendVertex(ptoff);
			
			GA_Offset ptoff2 = poly->getPointOffset(1);
			UT_Vector3F endPos = UT_Vector3F(branches[i].second[0], branches[i].second[2], branches[i].second[1]);
